...
use reth_optimism_trie::{
    MdbxProofsStorage, OpProofStoragePruner, OpProofsProviderRO, OpProofsStore,
};
use tracing::info;
...
impl<C: ChainSpecParser<ChainSpec = TaikoChainSpec>> PruneCommand<C> {
    ...
    pub async fn execute<N: CliNodeTypes<ChainSpec = C::ChainSpec, Primitives = EthPrimitives>>(
        ...
        // Use `OpProofStoragePruner` (the non-`Task` variant): a single-shot manual prune.
        // The `OpProofStoragePrunerTask` wrapper adds a periodic timer that is useful for a
        // long-running sidecar process, but pointless for this one-off CLI invocation.
