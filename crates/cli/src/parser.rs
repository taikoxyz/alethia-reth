...
mod test {
    ...
    #[test]
    fn test_masaya_network_names_are_not_supported() {
        for name in ["masaya", "taiko-masaya", "taiko masaya"] {
            assert!(
                TaikoChainSpecParser::parse(name).is_err(),
                "Masaya alias {name:?} must not resolve to a built-in chain spec"
            );
        }
    }
}
