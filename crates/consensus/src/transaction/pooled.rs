...
use crate::transaction::TaikoTxEnvelope;
...
///
/// The difference between this and the [`TaikoTxEnvelope`] is that this type does not have the
/// deposit transaction variant, which is not expected to be pooled.
///
/// Note: this struct is the same as TaikoTxEvelope for now, but they are kept separate for
/// future extensibility.
#[derive(Clone, Debug, TransactionEnvelope)]
#[envelope(tx_type_name = OpPooledTxType, serde_cfg(feature = "serde"))]
pub enum TaikoPooledTransaction {
...
impl TaikoPooledTransaction {
    ...
    /// Converts the transaction into the [`TaikoTxEnvelope`].
    pub fn into_taiko_envelope(self) -> TaikoTxEnvelope {
        match self {
            Self::Legacy(tx) => tx.into(),
            Self::Eip2930(tx) => tx.into(),
            Self::Eip1559(tx) => tx.into(),
...
impl From<Signed<TxLegacy>> for TaikoPooledTransaction {
    fn from(v: Signed<TxLegacy>) -> Self {
        Self::Legacy(v)
    }
}

impl From<Signed<TxEip2930>> for TaikoPooledTransaction {
    fn from(v: Signed<TxEip2930>) -> Self {
        Self::Eip2930(v)
    }
}

impl From<Signed<TxEip1559>> for TaikoPooledTransaction {
    fn from(v: Signed<TxEip1559>) -> Self {
        Self::Eip1559(v)
    }
}

impl From<Signed<TxEip7702>> for TaikoPooledTransaction {
    fn from(v: Signed<TxEip7702>) -> Self {
        Self::Eip7702(v)
    }
}

impl From<TaikoPooledTransaction> for alloy_consensus::transaction::PooledTransaction {
    fn from(value: TaikoPooledTransaction) -> Self {
        match value {
            TaikoPooledTransaction::Legacy(tx) => tx.into(),
            TaikoPooledTransaction::Eip2930(tx) => tx.into(),
            TaikoPooledTransaction::Eip1559(tx) => tx.into(),
            TaikoPooledTransaction::Eip7702(tx) => tx.into(),
        }
    }
}

impl TxHashRef for TaikoPooledTransaction {
...
#[cfg(feature = "k256")]
impl alloy_consensus::transaction::SignerRecoverable for TaikoPooledTransaction {
    fn recover_signer(
        &self,
...
impl From<TaikoPooledTransaction> for TxEnvelope {
    fn from(tx: TaikoPooledTransaction) -> Self {
        tx.into_envelope()
    }
}

impl From<TaikoPooledTransaction> for TaikoTxEnvelope {
    fn from(tx: TaikoPooledTransaction) -> Self {
        tx.into_taiko_envelope()
    }
}

impl TryFrom<TaikoTxEnvelope> for TaikoPooledTransaction {
    type Error = ValueError<TaikoTxEnvelope>;

    fn try_from(value: TaikoTxEnvelope) -> Result<Self, Self::Error> {
        value.try_into_pooled()
    }
}

impl<Tx> From<TaikoPooledTransaction> for Extended<TaikoTxEnvelope, Tx> {
    fn from(tx: TaikoPooledTransaction) -> Self {
        Self::BuiltIn(tx.into())
    }
}

impl<Tx> TryFrom<Extended<TaikoTxEnvelope, Tx>> for TaikoPooledTransaction {
    type Error = ();

    fn try_from(_tx: Extended<TaikoTxEnvelope, Tx>) -> Result<Self, Self::Error> {
        match _tx {
...
mod tests {
    ...
    fn invalid_legacy_pooled_decoding_input_too_short() {
        ...
        for hex_data in &input_too_short {
            ...
            let res = TaikoPooledTransaction::decode(input_rlp);
            ...
            let input_rlp = &mut &hex_data[..];
            let res = TaikoPooledTransaction::decode_2718(input_rlp);
    ...
    fn decode_eip1559_enveloped() {
        ...
        let res = TaikoPooledTransaction::decode_2718(&mut &data[..]).unwrap();
    ...
    fn legacy_valid_pooled_decoding() {
        ...
        let res = TaikoPooledTransaction::decode_2718(&mut &data[..]);
        assert!(res.is_ok());
    }
}
