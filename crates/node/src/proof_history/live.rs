...
/// Re-executes canonical blocks and persists their trie changes in proof-history storage.
///
/// The collector also applies precomputed canonical updates and atomically replaces or unwinds
/// retained history during reorganizations.
...
{
    /// Re-executes `block` against its stored parent state and appends the resulting trie changes.
    ///
    /// The block must extend the current proof-history tip, and its computed state root must match
    /// the header before any updates are written. Parent, execution, state-root, and storage
    /// failures are returned without committing a partial update.
    ...
    /// Appends precomputed trie and hashed-state updates for `block` in one storage transaction.
    ///
    /// The storage validates append ordering; a validation or commit failure is returned without
    /// reporting the block as stored.
    ...
    /// Removes account, storage, and trie updates from `to` onward and commits the unwind.
    ///
    /// Storage validation and commit failures are returned to the caller.
