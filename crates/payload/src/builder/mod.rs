...
/// Rejects payload construction when Amsterdam block access lists would be mandatory.
///
/// Taiko does not implement EIP-7928, so payload building must fail before accessing parent state
/// once Amsterdam is active at the requested timestamp.
        ...
        >,
{
    ...
    ensure_amsterdam_inactive(client.chain_spec().as_ref(), attributes.timestamp())?;
    ...
    // If we have a state-root task, wire a state hook that streams per-transaction state diffs.
    ...
    assert!(
        ...
        "Taiko block assembler produced an unsupported EIP-7928 block access list"
    );
