//! Versioned MDBX cursor implementations for historical trie and hashed-state lookups.
//!
//! Ported verbatim from op-reth's `crates/trie/src/db/cursor.rs`.
...
pub struct BlockNumberVersionedCursor<T: Table + DupSort, Cursor> {
    /// Phantom marker tying the cursor to the underlying [`Table`] type.
    _table: PhantomData<T>,
    /// The underlying MDBX dup-sort cursor.
    cursor: Cursor,
    /// Upper bound on the block number for version resolution (inclusive).
    ...
    Cursor: DbCursorRO<T> + DbDupCursorRO<T>,
{
    ...
    fn is_positioned(&mut self) -> ProofsStorageResult<bool> {
    ...
    fn latest_version_for_key(
        &mut self,
        key: T::Key,
    ) -> ProofsStorageResult<Option<(T::Key, T::Value)>> {
    ...
    fn seek_exact(&mut self, key: T::Key) -> ProofsStorageResult<Option<(T::Key, V)>> {
    ...
    fn next_live_from(
        &mut self,
        mut first_key: T::Key,
    ) -> ProofsStorageResult<Option<(T::Key, V)>> {
        loop {
    ...
    fn seek(&mut self, start_key: T::Key) -> ProofsStorageResult<Option<(T::Key, V)>> {
    ...
    fn next(&mut self) -> ProofsStorageResult<Option<(T::Key, V)>>
    where
        T::Key: Default,
    {
...
pub struct MdbxTrieCursor<T: Table + DupSort, Cursor> {
    /// The underlying versioned cursor driving the lookups.
    inner: BlockNumberVersionedCursor<T, Cursor>,
    /// Optional hashed account address used to scope storage-trie iteration.
...
pub struct MdbxStorageCursor<Cursor> {
    /// The underlying versioned cursor over [`HashedStorageHistory`].
    inner: BlockNumberVersionedCursor<HashedStorageHistory, Cursor>,
    /// Hashed account address this cursor is bound to.
    hashed_address: B256,
}
...
pub struct MdbxAccountCursor<Cursor> {
    /// The underlying versioned cursor over [`HashedAccountHistory`].
