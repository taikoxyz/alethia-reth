...
//!
//! This module provides the [`MdbxProofsStorage`] wrapper that owns the
//! underlying MDBX environment and performs the one-time schema-version check
//! during construction, along with [`MdbxProofsProvider`] implementations of
//! the read-only, read-write, and initialisation provider traits.

use super::Tables;
use crate::{
    BlockStateDiff, ProofsStorageError,
    ProofsStorageError::NoBlocksFound,
    ProofsStorageResult,
    api::{
        InitialStateAnchor, InitialStateStatus, ProofsInitProvider, ProofsProviderRO,
        ProofsProviderRw, ProofsStore, WriteCounts,
    },
    db::{
        BlockChangeSet, BlockNumberHash, MdbxAccountCursor, MdbxStorageCursor, MdbxTrieCursor,
        ProofWindow, ProofWindowKey,
        models::{
            AccountTrieHistory, ChangeSet, HashedAccountHistory, HashedStorageHistory,
            HashedStorageKey, MaybeDeleted, PROOFS_TRIE_SCHEMA_VERSION,
            PROOFS_TRIE_SCHEMA_VERSION_KEY, SchemaVersion, StorageTrieHistory, StorageTrieKey,
...
/// Proof window bounds returned by [`MdbxProofsProvider::get_proof_window_inner`].
struct ProofWindowValue {
    /// Earliest block (and hash) retained in the proof window.
    earliest: NumHash,
    /// Latest block (and hash) retained in the proof window.
    latest: NumHash,
}
...
#[derive(Debug, Clone)]
struct PrunePlan {
    /// Earliest block currently tracked by the proof window.
    earliest_block: u64,
    /// Survivor `(account-trie key, block)` pairs (latest version per key).
    acc_survivors: Vec<(StoredNibbles, u64)>,
    /// Survivor `(storage-trie key, block)` pairs (latest version per key).
    storage_survivors: Vec<(StorageTrieKey, u64)>,
    /// Survivor `(hashed-account key, block)` pairs (latest version per key).
    hashed_acc_survivors: Vec<(B256, u64)>,
    /// Survivor `(hashed-storage key, block)` pairs (latest version per key).
...
struct HistoryDeleteBatch {
    /// Account-trie history entries `(key, subkey)` scheduled for deletion.
    account_trie: Vec<(<AccountTrieHistory as Table>::Key, u64)>,
    /// Storage-trie history entries `(key, subkey)` scheduled for deletion.
    storage_trie: Vec<(<StorageTrieHistory as Table>::Key, u64)>,
    /// Hashed-account history entries `(key, subkey)` scheduled for deletion.
    hashed_account: Vec<(<HashedAccountHistory as Table>::Key, u64)>,
    /// Hashed-storage history entries `(key, subkey)` scheduled for deletion.
...
pub struct MdbxProofsProvider<TX> {
    /// The MDBX transaction this provider operates on.
    pub(crate) tx: TX,
}
...
impl<TX: DbTx> MdbxProofsProvider<TX> {
    /// Look up the `(block-number, block-hash)` pair stored under the given
    /// [`ProofWindowKey`], if any.
    fn get_block_number_hash_inner(
        &self,
        key: ProofWindowKey,
    ) -> ProofsStorageResult<Option<(u64, B256)>> {
        let mut cursor = self.tx.cursor_read::<ProofWindow>()?;
        let value = cursor.seek_exact(key)?;
        Ok(value.map(|(_, val)| (val.number(), *val.hash())))
    }

    /// Return the latest known `(block-number, block-hash)`. Falls back to the
    /// earliest pointer when the latest pointer is not set, and errors with
    /// [`ProofsStorageError::NoBlocksFound`] if neither exists.
    fn get_latest_block_number_hash_inner(&self) -> ProofsStorageResult<(u64, B256)> {
    ...
    fn calculate_prune_plan(&self, target_block: u64) -> ProofsStorageResult<Option<PrunePlan>> {
        let Some((earliest, _)) =
...
impl<TX: DbTxMut + DbTx> MdbxProofsProvider<TX> {
    /// Upsert the earliest-block pointer in the [`ProofWindow`] table.
    ...
    /// Upsert the latest-block pointer in the [`ProofWindow`] table.
    ...
    fn delete_dup_sorted<T, I, V>(&self, items: I) -> ProofsStorageResult<()>
    where
        T: Table<Value = VersionedValue<V>> + DupSort<SubKey = u64>,
        T::Key: Clone,
    ...
    ) -> ProofsStorageResult<HistoryDeleteBatch> {
    ...
    ) -> ProofsStorageResult<WriteCounts> {
    ...
    /// Append-only live-write path: validates parent linkage, persists the
    /// `BlockStateDiff` as versioned history at `block_ref.block.number`, and
    /// advances the latest-block pointer.
    fn store_trie_updates_append_only_inner(
        ...
        if latest_block_hash != block_ref.parent {
            return Err(ProofsStorageError::OutOfOrder {
                block_number,
    ...
    /// Read the earliest/latest proof-window pointers in a single pass.
    fn get_proof_window_inner(&self) -> ProofsStorageResult<ProofWindowValue> {
        let mut cursor = self.tx.cursor_read::<ProofWindow>()?;
    ...
    /// Read the stored [`ProofWindowKey::InitialStateAnchor`], if any.
    fn get_initial_state_anchor_inner(&self) -> ProofsStorageResult<Option<BlockNumHash>> {
        let mut cur = self.tx.cursor_read::<ProofWindow>()?;
        Ok(cur.seek_exact(ProofWindowKey::InitialStateAnchor)?.map(|(_k, v)| v.into()))
    }

    /// Return the lexicographically last key stored in table `T`, if any.
    fn get_latest_key_inner<T>(&self) -> ProofsStorageResult<Option<T::Key>>
    where
        T: Table,
    {
...
impl<TX: DbTx + Send + Sync + Debug + 'static> ProofsProviderRO for MdbxProofsProvider<TX> {
    ...
    fn get_earliest_block_number(&self) -> ProofsStorageResult<Option<(u64, B256)>> {
        let mut cursor = self.tx.cursor_read::<ProofWindow>()?;
        let value = cursor.seek_exact(ProofWindowKey::EarliestBlock)?;
        Ok(value.map(|(_, val)| (val.number(), *val.hash())))
    }

    fn get_latest_block_number(&self) -> ProofsStorageResult<Option<(u64, B256)>> {
        let mut cursor = self.tx.cursor_read::<ProofWindow>()?;
        // Try latest first
    ...
    ) -> ProofsStorageResult<Self::StorageTrieCursor<'tx>> {
        let cursor = self.tx.cursor_dup_read::<StorageTrieHistory>()?;
    ...
    fn account_trie_cursor<'tx>(
        &self,
        max_block_number: u64,
    ) -> ProofsStorageResult<Self::AccountTrieCursor<'tx>> {
    ...
    fn storage_hashed_cursor<'tx>(
        ...
        hashed_address: B256,
        max_block_number: u64,
    ) -> ProofsStorageResult<Self::StorageCursor<'tx>> {
        let cursor = self.tx.cursor_dup_read::<HashedStorageHistory>()?;
    ...
    fn account_hashed_cursor<'tx>(
        &self,
        max_block_number: u64,
    ) -> ProofsStorageResult<Self::AccountHashedCursor<'tx>> {
        let cursor = self.tx.cursor_dup_read::<HashedAccountHistory>()?;
        Ok(MdbxAccountCursor::new(cursor, max_block_number))
    }

    fn fetch_trie_updates(&self, block_number: u64) -> ProofsStorageResult<BlockStateDiff> {
        let mut change_set_cursor = self.tx.cursor_read::<BlockChangeSet>()?;
        let (_, change_set) = change_set_cursor
            .seek_exact(block_number)?
            .ok_or(ProofsStorageError::NoChangeSetForBlock(block_number))?;
        ...
        for key in change_set.account_trie_keys {
            let entry = match account_trie_cursor.seek_by_key_subkey(key.clone(), block_number)? {
                ...
                _ => {
                    return Err(ProofsStorageError::MissingAccountTrieHistory(key.0, block_number));
                }
            };
        ...
        for key in change_set.storage_trie_keys {
            let entry = match storage_trie_cursor.seek_by_key_subkey(key.clone(), block_number)? {
                Some(v) if v.block_number == block_number => v.value.0,
                _ => {
                    return Err(ProofsStorageError::MissingStorageTrieHistory(
                        key.hashed_address,
                        key.path.0,
                        block_number,
                    ));
        ...
        for key in change_set.hashed_account_keys {
            let entry = match hashed_account_cursor.seek_by_key_subkey(key, block_number)? {
                Some(v) if v.block_number == block_number => v.value.0,
                _ => {
                    return Err(ProofsStorageError::MissingHashedAccountHistory(key, block_number));
                }
            };
        ...
        for key in change_set.hashed_storage_keys {
            let entry = match hashed_storage_cursor.seek_by_key_subkey(key.clone(), block_number)? {
                Some(v) if v.block_number == block_number => v.value.0,
                _ => {
                    return Err(ProofsStorageError::MissingHashedStorageHistory {
...
impl<TX: DbTxMut + DbTx + Send + Sync + Debug + 'static> ProofsProviderRw
    for MdbxProofsProvider<TX>
{
    ...
    fn unwind_history(&self, to: BlockWithParent) -> ProofsStorageResult<()> {
        ...
        if to.block.number <= proof_window.earliest.number {
            return Err(ProofsStorageError::UnwindBeyondEarliest {
    ...
    fn set_earliest_block_number(&self, block_number: u64, hash: B256) -> ProofsStorageResult<()> {
        self.set_earliest_block_number_inner(block_number, hash)
    }

    fn commit(self) -> ProofsStorageResult<()> {
        self.tx.commit()?;
        Ok(())
    }
}

impl<TX: DbTxMut + DbTx + Send + Sync + Debug + 'static> ProofsInitProvider
    for MdbxProofsProvider<TX>
{
    fn initial_state_anchor(&self) -> ProofsStorageResult<InitialStateAnchor> {
    ...
    fn set_initial_state_anchor(&self, anchor: BlockNumHash) -> ProofsStorageResult<()> {
    ...
    fn commit_initial_state(&self) -> ProofsStorageResult<BlockNumHash> {
...
impl ProofsStore for MdbxProofsStorage {
    ...
    fn provider_ro<'a>(&'a self) -> ProofsStorageResult<Self::ProviderRO<'a>> {
        Ok(Arc::new(MdbxProofsProvider::new(self.env.tx()?)))
    }

    fn provider_rw<'a>(&'a self) -> ProofsStorageResult<Self::ProviderRw<'a>> {
        Ok(MdbxProofsProvider::new(self.env.tx_mut()?))
    }

    fn initialization_provider<'a>(&'a self) -> ProofsStorageResult<Self::Initializer<'a>> {
        Ok(MdbxProofsProvider::new(self.env.tx_mut()?))
    }
}
...
impl reth_db::database_metrics::DatabaseMetrics for MdbxProofsStorage {
    ...
    fn gauge_metrics(&self) -> Vec<(&'static str, f64, Vec<Label>)> {
        ...
        let _ = self
            ...
            .view(|tx| {
                for table in Tables::ALL.iter().map(Tables::name) {
                    ...
                    metrics.push((
                        "alethia_proofs_storage.table_size",
                        table_size as f64,
                        vec![Label::new("table", table)],
                    ));
                    metrics.push((
                        "alethia_proofs_storage.table_pages",
                        leaf_pages as f64,
                    ...
                    metrics.push((
                        "alethia_proofs_storage.table_entries",
                        entries as f64,
                        vec![Label::new("table", table)],
                    ));
                }
        ...
        {
            metrics.push(("alethia_proofs_storage.freelist", freelist as f64, vec![]));
        }

        if let Ok(stat) = self.env.stat().map_err(|error| error!(%error, "Failed to read db.stat"))
        {
            metrics.push(("alethia_proofs_storage.page_size", stat.page_size() as f64, vec![]));
        }

        metrics.push((
            "alethia_proofs_storage.timed_out_not_aborted_transactions",
...
mod tests {
    use super::*;
    use crate::{
        ProofsInitProvider, ProofsProviderRO, ProofsProviderRw,
        db::{
            StorageTrieKey,
    ...
    impl MdbxProofsStorage {
        fn store_hashed_accounts(
            ...
            ProofsInitProvider::commit(provider)
        }
        ...
        fn store_trie_updates(
            ...
            ProofsProviderRw::commit(provider)?;
            Ok(counts)
        }
    ...
    fn store_hashed_accounts_writes_versioned_values() {
        ...
        ProofsInitProvider::commit(init_provider).expect("commit");
    ...
    fn store_trie_updates_out_of_order_rejects() {
        ...
        assert!(matches!(res, Err(ProofsStorageError::OutOfOrder { .. })));
    ...
    fn fetch_trie_updates_missing_changeset_returns_error() {
        ...
        assert!(matches!(res, Err(ProofsStorageError::NoChangeSetForBlock(99))));
    }
    ...
    fn fetch_trie_updates_missing_account_history_entry_returns_error() {
        ...
        assert!(matches!(res, Err(ProofsStorageError::MissingAccountTrieHistory(..))));
    }
    ...
    fn fetch_trie_updates_missing_storage_history_entry_returns_error() {
        ...
        let res = store.fetch_trie_updates(1);
        assert!(matches!(res, Err(ProofsStorageError::MissingStorageTrieHistory(..))));
    }
    ...
    fn fetch_trie_updates_missing_hashed_account_entry_returns_error() {
        ...
        let res = store.fetch_trie_updates(1);
        assert!(matches!(res, Err(ProofsStorageError::MissingHashedAccountHistory(..))));
    }
    ...
    fn fetch_trie_updates_missing_hashed_storage_entry_returns_error() {
        ...
        let res = store.fetch_trie_updates(1);
        assert!(matches!(res, Err(ProofsStorageError::MissingHashedStorageHistory { .. })));
    }
    ...
    fn test_prune_earliest_state_uninitialized_guard() {
        ...
        assert!(
            matches!(result, Err(ProofsStorageError::NoBlocksFound)),
    ...
    fn test_unwind_history_to_earliest() {
        ...
        assert!(matches!(res.unwrap_err(), ProofsStorageError::UnwindBeyondEarliest { .. }));
    }
