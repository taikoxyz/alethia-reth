//! In-memory implementation of [`ProofsStore`] for testing purposes

use crate::{
    BlockStateDiff, ProofsStorageError, ProofsStorageResult, ProofsStore,
    api::{
...
/// Shared in-memory state backing [`InMemoryProofsStorage`] and its providers.
...
impl InMemoryStorageInner {
    /// Apply a [`BlockStateDiff`] to the in-memory state for the given block number and return
...
impl InMemoryProofsStorage {
    /// Create a new in-memory proofs storage instance
    pub fn new() -> Self {
...
impl InMemoryTrieCursor {
    /// Create a new [`InMemoryTrieCursor`]. If `hashed_address` is `Some`, the cursor iterates
    /// storage trie branches for that address; otherwise it iterates account trie branches.
    const fn new(
        inner: Arc<RwLock<InMemoryStorageInner>>,
        hashed_address: Option<B256>,
        max_block_number: u64,
    ...
    /// Populate `self.entries` with the latest branch node per path visible at
    /// `self.max_block_number`, sorted by path. No-op if already populated.
    fn ensure_entries_populated(&mut self) -> Result<(), DatabaseError> {
        if self.is_populated {
            return Ok(());
        }

        let storage = self.inner.try_read().ok_or(ProofsStorageError::TryLockError)?;
...
impl InMemoryStorageCursor {
    /// Create a new [`InMemoryStorageCursor`] for the given hashed address, visible up to
    /// `max_block_number`.
    const fn new(
    ...
    /// Populate `self.entries` with the latest non-zero storage value per slot visible at
    /// `self.max_block_number` for `self.hashed_address`, sorted by slot. No-op if already
    /// populated.
    fn ensure_entries_populated(&mut self) -> Result<(), DatabaseError> {
        if self.is_populated {
            return Ok(());
...
impl InMemoryAccountCursor {
    /// Create a new [`InMemoryAccountCursor`] snapshot from the given storage, materializing the
    /// latest existing account per hashed address visible at `max_block_number`.
...
impl ProofsStore for InMemoryProofsStorage {
...
impl InMemoryProofsProvider {
    /// Return the largest path stored for the account trie within the initial-state range
    /// (block 0), or `None` if no account trie branches have been written.
    fn get_latest_account_trie_key(&self) -> ProofsStorageResult<Option<StoredNibbles>> {
    ...
    /// Return the largest (address, path) stored for the storage trie within the initial-state
    /// range (block 0), or `None` if no storage trie branches have been written.
    fn get_latest_storage_trie_key(&self) -> ProofsStorageResult<Option<StorageTrieKey>> {
        let inner = self.inner.read();
        Ok(inner
            .storage_branches
            .range((
    ...
    /// Return the largest hashed account address stored within the initial-state range
    /// (block 0), or `None` if no hashed accounts have been written.
    fn get_latest_hashed_account_key(&self) -> ProofsStorageResult<Option<B256>> {
        let inner = self.inner.read();
        Ok(inner
            .hashed_accounts
            .range((
    ...
    /// Return the largest (address, slot) stored for hashed storage within the initial-state
    /// range (block 0), or `None` if no hashed storage entries have been written.
    fn get_latest_hashed_storage_key(&self) -> ProofsStorageResult<Option<HashedStorageKey>> {
        let inner = self.inner.read();
        Ok(inner
            .hashed_storages
            .range((
...
impl ProofsProviderRO for InMemoryProofsProvider {
    ...
    fn account_hashed_cursor<'tx>(
        ...
        let inner = self.inner.try_read().ok_or(ProofsStorageError::TryLockError)?;
...
impl ProofsProviderRw for InMemoryProofsProvider {
    fn store_trie_updates(
        &self,
    ...
    fn unwind_history(&self, unwind_upto_block: BlockWithParent) -> ProofsStorageResult<()> {
        let mut inner = self.inner.write();
...
impl ProofsInitProvider for InMemoryProofsProvider {
    ...
    fn commit_initial_state(&self) -> ProofsStorageResult<BlockNumHash> {
        let mut inner = self.inner.write();
        let anchor = inner.anchor_block.ok_or(ProofsStorageError::NoBlocksFound)?;
...
mod tests {
    use super::*;
    use crate::ProofsStorageError;
    use alloy_eips::NumHash;
    use alloy_primitives::U256;
    use reth_primitives_traits::Account;

    #[test]
    fn test_in_memory_storage_basic_operations() -> Result<(), ProofsStorageError> {
    ...
    #[test]
    fn test_trie_updates_storage() -> Result<(), ProofsStorageError> {
        let storage = InMemoryProofsStorage::new();
