...
use crate::{
    ProofsStorageError, ProofsStore,
    api::{InitialStateAnchor, InitialStateStatus, ProofsInitProvider},
...
pub struct InitializationJob<Tx: DbTx, S: ProofsStore + Send> {
    /// Proofs sidecar storage to initialize.
    storage: S,
    /// Read transaction on the main reth database from which the baseline state is copied.
    tx: Tx,
}
...
impl<Tx: DbTx + Sync, S: ProofsStore + Send> InitializationJob<Tx, S> {
    ...
    fn initialize_hashed_accounts(
        ...
        if let Some(latest) = start_key {
            start_cursor
                .seek(latest)?
                .filter(|(k, _)| *k == latest)
                .ok_or(ProofsStorageError::InitializeStorageInconsistentState)?;
        }
    ...
    fn initialize_trie(&self, anchor: InitialStateAnchor) -> Result<(), ProofsStorageError> {
    ...
    /// Validate that the anchor stored in the sidecar matches the caller-provided best block,
    /// returning [`ProofsStorageError::InitializeStorageInconsistentState`] otherwise.
    fn validate_anchor_block(
        ...
        let block = anchor.block.ok_or(ProofsStorageError::InitializeStorageInconsistentState)?;

        if block.number != best_number || block.hash != best_hash {
            return Err(ProofsStorageError::InitializeStorageInconsistentState);
        }
    ...
    pub fn run(&self, best_number: u64, best_hash: B256) -> Result<(), ProofsStorageError> {
...
mod tests {
    use super::*;
    use crate::{MdbxProofsStorage, ProofsProviderRO};
    ...
    use crate::ProofsStorageResult;
    use std::sync::Mutex;

    /// Recording wrapper around any [`ProofsInitProvider`] that logs the
    ...
    impl<T: ProofsInitProvider> ProofsInitProvider for RecordingInitProvider<T> {
    ...
    impl ProofsStore for RecordingStore {
        type ProviderRO<'a> = <MdbxProofsStorage as ProofsStore>::ProviderRO<'a>;
        type ProviderRw<'a> = <MdbxProofsStorage as ProofsStore>::ProviderRw<'a>;
        type Initializer<'a> =
            RecordingInitProvider<<MdbxProofsStorage as ProofsStore>::Initializer<'a>>;
