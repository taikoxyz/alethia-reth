...
use crate::{
    BlockStateDiff, ProofsStorageError, ProofsStore,
    api::{OperationDurations, ProofsProviderRO, ProofsProviderRw},
    metrics::ProofsStorage,
    provider::ProofsStateProviderRef,
};
...
{
    /// EVM configuration used to build executors for incoming blocks.
    evm_config: Evm,
    /// Reth state provider factory used to obtain the parent-block state.
    provider: Provider,
    /// Proofs storage where derived trie updates are persisted.
    storage: &'tx ProofsStorage<PreimageStore>,
}
...
where
    Evm: ConfigureEvm,
    Provider: StateReader + DatabaseProviderFactory + StateProviderFactory,
    Store: 'tx + ProofsStore + Clone + 'static,
{
    ...
    pub fn execute_and_store_block_updates(
        ...
        let (Some((earliest, _)), Some((latest, _))) =
            ...
            return Err(ProofsStorageError::NoBlocksFound);
        };

        let parent_block_number = block.number() - 1;
        if parent_block_number < earliest {
            return Err(ProofsStorageError::UnknownParent);
        }

        if parent_block_number > latest {
            return Err(ProofsStorageError::MissingParentBlock {
        ...
        let state_provider = ProofsStateProviderRef::new(
        ...
        if state_root != block.state_root() {
            return Err(ProofsStorageError::StateRootMismatch {
                block_number: block.number(),
    ...
    pub fn unwind_history(&self, to: BlockWithParent) -> Result<(), ProofsStorageError> {
        let provider_rw = self.storage.provider_rw()?;
