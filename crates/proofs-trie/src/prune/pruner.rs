//! Window-based pruner for the proofs storage.

use crate::{
    ProofsStore,
    api::{ProofsProviderRO, ProofsProviderRw},
    prune::{
        error::{ProofsStoragePrunerResult, PrunerError, PrunerOutput},
        metrics::Metrics,
    },
};
...
#[derive(Debug)]
pub struct ProofsStoragePruner<P, H> {
    ...
    /// Metrics collector for pruner runs.
    #[doc(hidden)]
    metrics: Metrics,
}

impl<P, H> ProofsStoragePruner<P, H> {
    /// Create a new pruner.
    pub fn new(
        provider: P,
        block_hash_reader: H,
...
where
    ...
    H: BlockHashReader,
{
    /// Core pruning logic: walk the stored block window and prune batches
    /// until the target earliest block is reached.
    fn run_inner(&self) -> ProofsStoragePrunerResult {
...
mod tests {
    ...
    /// Simple hand-rolled mock for `BlockHashReader` used in pruner tests.
    #[derive(Debug, Default)]
    struct MockBlockHashReader {
        hashes: HashMap<BlockNumber, B256>,
    }

    impl MockBlockHashReader {
        fn new() -> Self {
            Self::default()
        }
    ...
    impl BlockHashReader for MockBlockHashReader {
    ...
    async fn run_inner_and_and_verify_updated_state() {
        ...
        block_hash_reader.insert(4, b256(4));
        block_hash_reader.insert(3, b256(3));

        let pruner = ProofsStoragePruner::new(store.clone(), block_hash_reader, 1, 1000);
    ...
    async fn run_inner_where_latest_block_is_none() {
        ...
        let pruner = ProofsStoragePruner::new(store, block_hash_reader, 10, 1000);
    ...
    async fn run_inner_earliest_none_real_db() {
        ...
        let block_hash_reader = MockBlockHashReader::new();
        let pruner = ProofsStoragePruner::new(store, block_hash_reader, 1, 1000);
        let out = pruner.run_inner().expect("ok");
        assert_eq!(out, PrunerOutput::default(), "should early-return default output");
    }
    ...
    async fn run_inner_interval_too_small_real_db() {
        ...
        let block_hash_reader = MockBlockHashReader::new();
        let pruner = ProofsStoragePruner::new(store, block_hash_reader, 2, 1000);
        let out = pruner.run_inner().expect("ok");
