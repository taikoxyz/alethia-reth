//! Periodic pruner task wrapper for the proofs storage.

use crate::{ProofsStore, prune::ProofsStoragePruner};
...
/// Maximum number of blocks to prune in a single pruner batch/transaction.
const PRUNE_BATCH_SIZE: u64 = 200;

/// Periodic pruner task: constructs the pruner and runs it every interval.
#[derive(Debug)]
pub struct ProofsStoragePrunerTask<P, H> {
    /// The underlying pruner driving each periodic run.
    pruner: ProofsStoragePruner<P, H>,
    /// Minimum number of recent blocks to retain in the proof storage.
    min_block_interval: u64,
    /// Interval between successive pruner runs.
    task_run_interval: Duration,
}

impl<P, H> ProofsStoragePrunerTask<P, H>
where
    P: ProofsStore,
    H: BlockHashReader,
{
    /// Initialize a new [`ProofsStoragePrunerTask`]
    pub fn new(
        provider: P,
        hash_reader: H,
        min_block_interval: u64,
        ...
        let pruner =
            ProofsStoragePruner::new(provider, hash_reader, min_block_interval, PRUNE_BATCH_SIZE);
