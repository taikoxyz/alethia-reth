    ...
    async fn execution_witness(&self, block_id: BlockNumberOrTag) -> RpcResult<ExecutionWitness> {
        ...
        let _ = block_executor
            .execute_with_state_closure(block.as_ref(), |statedb: &State<_>| {
                // reth v2.1 added an `ExecutionWitnessMode` parameter to
                // witness recording. Use the default (legacy) mode, matching
                // op-reth's override and reth's own default.
        ...
        // v2.1 `witness` signature also requires an `ExecutionWitnessMode`.
        let state = state_provider
            .witness(Default::default(), hashed_state, ExecutionWitnessMode::default())
            .map_err(EthApiError::from)?;
...
mod tests {
    ...
    // asserting the resulting witness — lives in Task 21. Unit-testing the
