...
mod tests {
    ...
    // asserting the resulting proof — lives in `tests/proofs_history_e2e.rs`.
    // Unit-testing the handler in isolation would require mocking
    // `EthApiSpec`, `EthTransactions`, `EthBlocks`, `EthState`, `EthCall`,
    // `EthFees`, `Trace`, `LoadReceipt`, `GetBlockAccessList`,
    // `FullEthApiTypes` — prohibitively expensive for a unit test.
}
