...
//!
//! Taiko provenance note: upstream replaced this module with the `engine` service between the
//! previously pinned revision (`bcf489ea`) and the vendored revision (`9b802fdb`). Alethia's
//! proof-history sidecar drives collection itself, so the collector is retained here, ported to
//! the current storage API (`get_proof_window`).
