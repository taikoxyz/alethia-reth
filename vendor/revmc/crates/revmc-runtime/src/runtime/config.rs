...
pub struct RuntimeConfig {
    ...
    /// Collapses every JIT failure path to a single out-of-gas result when enabled.
    ///
    /// Disable this when callers require exact interpreter failure reasons and ordering.
    /// Defaults to `true` for compatibility with revmc's optimized runtime behavior.
