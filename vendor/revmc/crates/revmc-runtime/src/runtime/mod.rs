...
pub(crate) struct BackendInner {
    ...
    /// Global enable flag.
    enabled: AtomicBool,
    /// Whether the backend thread has started and can drain control commands.
    started: AtomicBool,
    ...
    /// Serializes pause-depth transitions without blocking concurrent callers.
    #[debug(skip)]
    control: std::sync::Mutex<()>,
    ...
    #[debug(skip)]
    spawn_hook:
        std::sync::Mutex<Option<Arc<dyn Fn() -> std::io::Result<()> + Send + Sync + 'static>>>,
}
...
/// Error returned when a runtime control command cannot be delivered immediately.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ControlError {
    /// The backend worker has not started and cannot receive commands.
    NotStarted,
    /// JIT execution is disabled.
    Disabled,
    /// Another pause-depth transition is currently in progress.
    Busy,
    /// The bounded command channel has no available capacity.
    QueueFull,
    /// The backend worker disconnected from the command channel.
    Disconnected,
}

impl std::fmt::Display for ControlError {
    /// Formats an actionable description of the failed control delivery.
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::NotStarted => "JIT backend is not enabled; worker has not started",
            Self::Disabled => "JIT backend is disabled",
            Self::Busy => "another JIT control operation is in progress",
            Self::QueueFull => "JIT backend control queue is full",
            Self::Disconnected => "JIT backend control channel is disconnected",
        })
    }
}
...
impl JitBackend {
    ...
    pub fn clear_resident(&self) {
        let _ = self.try_send_control(Command::ClearResident);
    }

    /// Clears persisted artifacts from the artifact store.
    pub fn clear_persisted(&self) {
        let _ = self.try_send_control(Command::ClearPersisted);
    }

    /// Clears both the resident map and persisted artifacts.
    pub fn clear_all(&self) {
        let _ = self.try_clear_all();
    }

    /// Clears resident and persisted artifacts without blocking for channel capacity.
    pub fn try_clear_all(&self) -> Result<(), ControlError> {
        self.try_send_control(Command::ClearAll)
    }
    ...
    pub fn pause(&self) {
        let _ = self.try_pause();
    }

    /// Pauses helper execution without blocking for channel capacity or a control lock.
    pub fn try_pause(&self) -> Result<(), ControlError> {
        self.ensure_control_ready()?;
        let _guard = self.inner.control.try_lock().map_err(|_| ControlError::Busy)?;
        let depth = self.inner.shared.pause_depth.load(Ordering::Relaxed);
        if depth == 0 {
            self.try_send_control(Command::Pause)?;
        }
        self.inner.shared.pause_depth.store(depth.saturating_add(1), Ordering::Relaxed);
        Ok(())
    }
    ...
    /// Resumes helper execution without blocking for channel capacity or a control lock.
    pub fn try_resume(&self) -> Result<(), ControlError> {
        self.ensure_control_ready()?;
        let _guard = self.inner.control.try_lock().map_err(|_| ControlError::Busy)?;
        let depth = self.inner.shared.pause_depth.load(Ordering::Relaxed);
        if depth == 0 {
        ...
        self.inner.shared.pause_depth.store(depth - 1, Ordering::Relaxed);
        Ok(())
    }

    /// Sends a control command without waiting for the bounded channel.
    fn try_send_control(&self, command: Command) -> Result<(), ControlError> {
        self.ensure_control_ready()?;
        self.inner.tx.try_send(command).map_err(|error| match error {
            chan::TrySendError::Full(_) => ControlError::QueueFull,
            chan::TrySendError::Disconnected(_) => ControlError::Disconnected,
        })
    }

    /// Validates that a worker is running and JIT controls are enabled.
    fn ensure_control_ready(&self) -> Result<(), ControlError> {
    ...
    fn ensure_started(&self) -> eyre::Result<()> {
        ...
        let thread = match self.spawn_backend_thread(shared, thread_rx, thread_config, done_tx) {
            Ok(thread) => thread,
            Err(error) => {
                *guard = Some(LazySpawnState { rx, config });
                return Err(error).wrap_err("failed to spawn backend thread");
            }
        };
    ...
    /// Spawns the worker after lifecycle state has been reserved by [`Self::ensure_started`].
    fn spawn_backend_thread(
        ...
        #[cfg(test)]
        if let Some(hook) = self.inner.spawn_hook.lock().unwrap().clone() {
            hook()?;
        }

        std::thread::Builder::new().name(config.thread_name.clone()).spawn(move || {
