...
#[test]
fn controls_reject_a_backend_that_has_not_started() {
    let backend = JitBackend::disabled();

    assert_eq!(backend.try_pause(), Err(ControlError::NotStarted));
    assert_eq!(backend.try_resume(), Err(ControlError::NotStarted));
    assert_eq!(backend.try_clear_all(), Err(ControlError::NotStarted));
    assert!(!backend.is_paused());
}

#[test]
fn controls_report_a_full_queue_without_changing_pause_depth() {
    let backend = JitBackend::new(RuntimeConfig {
        tuning: RuntimeTuning { channel_capacity: 1, ..RuntimeTuning::default() },
        ..RuntimeConfig::default()
    })
    .unwrap();

    backend.inner.started.store(true, Ordering::Relaxed);
    backend.inner.enabled.store(true, Ordering::Relaxed);
    assert!(backend.inner.tx.try_send(Command::ClearAll).is_ok());

    assert_eq!(backend.try_pause(), Err(ControlError::QueueFull));
    assert_eq!(backend.try_clear_all(), Err(ControlError::QueueFull));
    assert!(!backend.is_paused());

    let guard = backend.inner.lazy_spawn.lock().unwrap();
    assert!(guard.as_ref().unwrap().rx.try_recv().is_ok());
    backend.inner.enabled.store(false, Ordering::Relaxed);
    backend.inner.started.store(false, Ordering::Relaxed);
}

#[test]
fn controls_report_a_disconnected_worker() {
    let backend = JitBackend::disabled();
    drop(backend.inner.lazy_spawn.lock().unwrap().take());
    backend.inner.started.store(true, Ordering::Relaxed);
    backend.inner.enabled.store(true, Ordering::Relaxed);

    assert_eq!(backend.try_clear_all(), Err(ControlError::Disconnected));
}

#[test]
fn concurrent_enable_waits_for_failed_spawn_and_retries() {
    let backend = JitBackend::disabled();
    let spawn_entered = Arc::new(Barrier::new(2));
    let release_spawn = Arc::new(Barrier::new(2));
    let attempts = Arc::new(AtomicUsize::new(0));

    let hook_entered = Arc::clone(&spawn_entered);
    let hook_release = Arc::clone(&release_spawn);
    let hook_attempts = Arc::clone(&attempts);
    *backend.inner.spawn_hook.lock().unwrap() = Some(Arc::new(move || {
    ...
    let first = std::thread::spawn(move || {
        first_tx.send(first_backend.set_enabled(true)).unwrap();
    });
    spawn_entered.wait();
    ...
    let second = std::thread::spawn(move || {
        second_tx.send(second_backend.set_enabled(true)).unwrap();
    });
    assert!(
        second_rx.recv_timeout(std::time::Duration::from_millis(100)).is_err(),
        "the concurrent enable must wait while worker startup is unresolved",
    );
    ...
    let _ = first_rx
        ...
        .unwrap()
        .expect_err("the injected first spawn must fail");
    second_rx
        .recv_timeout(std::time::Duration::from_secs(5))
        .unwrap()
        .expect("the waiting enable must retry with the restored lifecycle state");

    first.join().unwrap();
    second.join().unwrap();
    assert!(backend.enabled());
    assert!(backend.inner.started.load(Ordering::Relaxed));
